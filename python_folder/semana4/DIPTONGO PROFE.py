...
def hay_diptongo(palabra):
    ...
    >>> hay_diptongo("búho")
    False
    >>> hay_diptongo('náutico')
    True
    >>> hay_diptongo('quiero')
    True
    >>> hay_diptongo('estudiáis')
    False
    >>> hay_diptongo('paraguay')
    False
    >>> hay_diptongo('opioide')
    False
    >>> hay_diptongo('buey')
    False
...
# Armado de la lista de posibles diptongos segun definicion
    ...
    for letra_1 in ["u", "i"]:
        for letra_2 in ["a", "e", "o", "á", "é", "ó"]:
            ...
            l_triptongo.append(letra_1+letra_2+letra_1)
    
    l_triptongo.extend(['uay','uey'])
    l_diptongo.extend(["ui", "iu"])
    
    #Control de si alguno de los diptongos forma parte de la palabra
    devolver = True
    posicion = 0
    while posicion < len(l_triptongo) and l_triptongo[posicion] not in palabra :
        posicion += 1
        if posicion == len(l_triptongo):
            devolver = False

    if devolver == True:        #si es triptongo termino e informo diptongo False
        return not devolver
