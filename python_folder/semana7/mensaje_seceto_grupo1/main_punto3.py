...
def recuperar_mensajes():
  with open('./semana7/mensaje_seceto_grupo1/mensajes.csv', mode='r', newline='') as archivo:
    reader=csv.reader(archivo)
    mensajes=[]
    for mensaje in reader:
      if mensaje[1]=='A':
        mensaje_descifrado = descifrar_mensaje(mensaje[2],'1',None)
      else:
        mensaje_descifrado = descifrar_mensaje(mensaje[3],'2',int(mensaje[2]))

      mensajes.append([mensaje[0],mensaje_descifrado])     
  return mensajes

def verificar_destinatario_espia(destinatario):
  mensajes=recuperar_mensajes()
  i=0
  alerta=False
  while  i<len(mensajes) and not alerta:
    if destinatario == mensajes[i][0]:
      alerta=True
    i+=1 
  return alerta

def informe(destinatario,mensajes_ordenados):
  print('Lista de Mensajes')
  for identificador,mensaje in mensajes_ordenados:
    print('------------------------------------------------------------------')
    if identificador==destinatario:
      print('%s : %s'%(identificador[0:3],mensaje))
    else:
      print('TODOS: %s' %mensaje)
  print('------------------------------------------------------------------\nTotal de Mensajes : %s'%len(mensajes_ordenados))

  return

def recobrar_mensaje():
  destinatario = input("Ingrese su nombre de espia para ver los mensaje: ")
  if verificar_destinatario(destinatario):
    if verificar_destinatario_espia(destinatario):
        print("Acceso Valido\n")
        mensajes=recuperar_mensajes()
        mensajes=sorted(mensajes,key=lambda item : item[0]==destinatario)
        informe(destinatario,mensajes)
    else:
